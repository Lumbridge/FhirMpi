...
public static class Hl7AcknowledgementFactory
{
        ...
        string? fallbackControlId = null)
    {
        ...
        return string.Join(
            "\r",
            $"MSH|^~\\&|FhirMpi|MPI|{Escape(metadata?.SendingApplication ?? string.Empty)}|{Escape(metadata?.SendingFacility ?? string.Empty)}|{timestamp}||ACK^{metadata?.TriggerEvent ?? "A00"}|{acknowledgementId}|P|{version}",
