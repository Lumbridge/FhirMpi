...
using AppAuthenticationOptions = OpenMpi.Api.AuthenticationOptions;
...
builder.Services.AddSingleton<IReadOnlyDictionary<OpenMpi.Domain.TenantId, OpenMpi.Domain.TenantMatchingConfiguration>>(
...
builder.Services.AddOpenTelemetry()
    .ConfigureResource(resource => resource.AddService("OpenMpi.Api"))
    .WithTracing(tracing => tracing
        .AddSource("OpenMpi.Registry")
        .AddAspNetCoreInstrumentation()
        .AddHttpClientInstrumentation()
        .AddOtlpExporter())
    .WithMetrics(metrics => metrics
        .AddMeter("OpenMpi.Registry")
        .AddAspNetCoreInstrumentation()
        .AddHttpClientInstrumentation()
        .AddOtlpExporter());
...
static Dictionary<OpenMpi.Domain.TenantId, OpenMpi.Domain.TenantMatchingConfiguration>
    BuildTenantConfigurations(TenantRegistryOptions options)
{
    var result =
        new Dictionary<OpenMpi.Domain.TenantId, OpenMpi.Domain.TenantMatchingConfiguration>();
    foreach (var item in options.Items)
    {
        var tenantId = new OpenMpi.Domain.TenantId(item.TenantId);
        var secrets = item.BlockingSecrets.Select(secret =>
        {
            ...
            return new OpenMpi.Domain.BlockingKeySecret(
                secret.Version,
                value,
                secret.Active);
        }).ToArray();
        ...
        var profile = OpenMpi.Domain.MatchingProfile.UkDefault with
        {
        ...
        var trust = item.SourceTrust.ToDictionary(
            static pair => new OpenMpi.Domain.SourceSystemId(pair.Key),
            static pair => pair.Value);
        var authoritativeSources = item.AuthoritativeSources
            .Select(static source => new OpenMpi.Domain.SourceSystemId(source))
            .ToHashSet();
        ...
        result.Add(
            tenantId,
            new OpenMpi.Domain.TenantMatchingConfiguration(
                tenantId,
                profile,
                secrets,
                trust,
